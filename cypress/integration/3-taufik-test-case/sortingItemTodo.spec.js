...
context("Fungsional: Dapat menampilkan sort terlama", () => {
    it("Sorting Data Set Automatically", () => {
      ...
      cy.wait(2000);
      cy.get("[data-cy=false]").eq(0).click();
    });
  });
context("Fungsional: Dapat menampilkan sort A-Z", () => {
  it("Sorting Data Set Automatically", () => {
    cy.get("[data-cy=todo-sort-button]").click();
    cy.wait(2000);
    cy.get("[data-cy=false]").eq(1).click();
  });
});
context("Fungsional: Dapat menampilkan sort Z-A", () => {
  it("Sorting Data Set Automatically", () => {
    cy.get("[data-cy=todo-sort-button]").click();
    cy.wait(2000);
    cy.get("[data-cy=false]").eq(2).click();
  });
  it("Checking last sort setting", () => {
    cy.get("[data-cy=todo-sort-button]").click();
    cy.wait(2000);
    cy.get("[data-cy=false]").eq(2).click();
  });
